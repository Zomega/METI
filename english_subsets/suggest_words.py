...
def load_glove_model():
    ...
    file_path = hf_hub_download(repo_id="JeremiahZ/glove", filename="glove.6B.50d.txt")
...
# Load Ogden words to filter them out of suggestions
def get_basic_words():
    import pandas as pd
    ...
    for f in files:
        try:
            ...
            basic_words.update(df["word"].dropna().str.lower().unique())
        except:
            pass
    return basic_words
...
def suggest_complex(simple_phrase, top_n=10):
    ...
    for word, score in neighbors:
        ...
        if word_lower in parts: continue
        if word_lower in basic_english_set: continue
        ...
        for p in parts:
            if p in word_lower or word_lower in p:
                if abs(len(p) - len(word_lower)) <= 2:
                    is_inflection = True
                    break
        if is_inflection: continue
        candidates.append(word)
    
    # 2. RERANKING (Transformer)
    # We re-rank a larger pool now
    pool_size = 300
    print(f"Reranking {len(candidates[:pool_size])} candidates for '{simple_phrase}'...")
